...
static void parse_conf_file(const char *conf_path)
{
    if (conf_path == NULL || conf_path[0] == '\0') {
        write_log(LOG_ERR, "bad conf path");
        return;
    }

    FILE *fp = fopen(conf_path, "r");
    if (fp == NULL) {
        write_log(LOG_ERR, "fopen[%s] failed with errno[%d]", conf_path, errno);
        return;
    }
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        ...
        while (isspace(*q)) q--;
        if (q < p) {
            write_log(LOG_WARNING, "bad config line[%s]", buf);
            continue;
        }
        if (*q == '\n') *q = '\0'; else *++q = '\0';

        if ((q = strchr(p, ':')) == NULL) {
            write_log(LOG_WARNING, "bad config line[%s] without a topic", buf);
            continue;
        }
        *q = '\0';
        raw_paths[p] = q + 1;
    ...
    if (ferror(fp) != 0) {
        write_log(LOG_ERR, "fgets[%s] failed", conf_path);
        raw_paths.clear();
    }
...
static std::string ymd_stringify(time_t t)
{
    ...
    if (localtime_r(&t, &tm) == NULL) {
        write_log(LOG_ERR, "localtime_r failed");
        return ymd;
    }
    char s[9];
...
static inline int set_nonblocking(int fd)
{
    ...
    if (old == -1) {
        write_log(LOG_ERR, "fcntl F_GETFL failed with errno[%d]", errno);
        return -1;
    }
    if (fcntl(fd, F_SETFL, old | O_NONBLOCK) == -1) {
        write_log(LOG_ERR, "fcntl F_SETFL failed with errno[%d]", errno);
        return -1;
    }
    return 0;
}
...
static int preprocess_inotify()
{
    ...
    if (inot_fd == -1) {
        write_log(LOG_ERR, "inotify_init failed with errno[%d]", errno);
        return -1;
    }
    ...
    for (std::multimap<std::string, std::string>::const_iterator it =
        ...
        if (wd == -1) {
            int errno_sv = errno;
            if (errno_sv != ENOENT) {
                write_log(LOG_ERR, "inotify_add_watch[%s] failed "
                          "with errno[%d]", it->second.c_str(), errno_sv);
                close(inot_fd);
                return -1;
            }
            write_log(LOG_WARNING, "inotify_add_watch[%s] failed "
                      "with errno[%d]", it->second.c_str(), errno_sv);
        } else {
                                        ...
                                        long offset = 0)
{
    ...
    if (fp == NULL) {
        write_log(LOG_ERR, "fopen[%s] for producing messages failed"
                  " with errno[%d]", fullpath, errno);
        return -1;
    }
    if (offset != 0) {
        if (fseek(fp, offset, SEEK_SET) != 0) {
            write_log(LOG_ERR, "fseek[%s] failed with errno[%d]",
                      fullpath, errno);
            fclose(fp);
            return -1;
        }
    ...
    char buf[5120], *p = strrchr(fullpath, '/');
    ...
    *p = '\0';
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        ...
        if (p2 != NULL) *p2 = '\0';
        if (buf[0] == '\0') {
            write_log(LOG_WARNING, "empty log line in dir[%s] file[%s]",
        ...
        if (++num % batch == 0) {
            if (produce_messages(producer, kct, payloads, keys) < 0) {
                write_log(LOG_ERR, "produce_messages for topic[%s] failed",
                          topic);
            } else {
                payloads.clear();
                fo.offset = ftell(fp);
                ...
                try {
                    ...
                    write_log(LOG_ERR, "saving offset in mem failed"
                              " with exception[%s]", e.what());
                } catch (...) {
                    write_log(LOG_ERR, "saving offset in mem failed"
    ...
    if (num % batch != 0) {
        if (produce_messages(producer, kct, payloads, keys) < 0) {
            write_log(LOG_ERR, "produce_messages failed");
        } else {
            fo.offset = ftell(fp);
            pthread_rwlock_rdlock(&offset_lock);
            std::map<std::string, FileOffset>::iterator it =
    ...
    *p = '/';
    write_log(LOG_INFO, "log sent [%s][%d]", fullpath, num);
...
static bool extract_topic_from_path(const std::string& path, std::string& topic)
{
    if (path.length() <= LOG_PATH_ROOT_LEN) {
        write_log(LOG_WARNING, "invalid path[%s] for extracting topic",
                  path.c_str());
        return false;
    }
...
{
    write_log(LOG_INFO, "thread handle_conveyor_backlog created");

    int ret = pthread_detach(pthread_self());
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_detach[handle_conveyor_backlog] failed"
                  " with errno[%d]", ret);
        write_log(LOG_ERR, "thread handle_conveyor_backlog exiting");
        return (void *)-1;
    }
    ...
    do {
        ...
        if (topics.size() < 60) continue;
        pthread_t thr;
        if ((ret = pthread_create(&thr, NULL, foo, NULL)) != 0) {
            write_log(LOG_ERR, "pthread_create[foo] for speedup failed"
                      " with errno[%d]", ret);
            continue;
        }
        producer_thread_total++;
        write_log(LOG_INFO, "creating one more thread[%d] foo for speedup",
                  producer_thread_total);
    } while (true);
...
static int baz(int inot_fd)
{
    ...
    do {
        ...
        if (n == -1) {
            if (errno == EAGAIN) {
                write_log(LOG_INFO, "reading inotify FD would block");
            } else {
                write_log(LOG_ERR, "reading inotify FD failed with errno[%d]");
                num = -1;
            }
            break;
        } else if (n == 0) {
            write_log(LOG_INFO, "no inotify events read");
            break;
        }
            ...
            if ((evp->mask & (IN_ISDIR | IN_CREATE)) != 0) {
                ...
                for (; it != normal_paths.end(); it++) {
                    ...
                    if (wd == -1) {
                        int errno_sv = errno;
                        if (errno_sv == ENOENT) {
                            write_log(LOG_WARNING, "inotify_add_watch[%s] "
                                      "failed with errno[%d] at midday",
                                      it->second.c_str(), errno_sv);
                        } else {
                            write_log(LOG_ERR, "inotify_add_watch[%s] "
                                      "failed with errno[%d] at midday",
                                      it->second.c_str(), errno_sv);
                        }
                        continue;
                    ...
                    write_log(LOG_INFO, "path[%s] might be newly watched"
                              " at midday", it->second.c_str());
                }
                ...
                if (evp->mask != IN_IGNORED) {
                    write_log(LOG_WARNING, "unexpected inotify event mask[%u]",
                              evp->mask);
                }
                continue;
            }
...
#define OFFSET_ARCHIVE_PATH (CWD_ROOT "/log_aggregator.offset")

static void persist_offsets()
{
    FILE *fp = fopen(OFFSET_ARCHIVE_PATH, "w");
    if (fp == NULL) {
        write_log(LOG_ERR, "fopen[%s] failed with errno[%d]",
    ...
    while (it != path_offset_table.end()) {
        ...
        int n = fputs(temp, fp);
        if (n == EOF) {
            write_log(LOG_ERR, "fputs[%s] for persisting offsets failed", temp);
        }
        it++;
    }
...
{
    write_log(LOG_INFO, "thread archive_offsets created");

    int ret = pthread_detach(pthread_self());
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_detach[archive_offsets] failed"
                  " with errno[%d]", ret);
        write_log(LOG_ERR, "thread archive_offsets exiting");
        return (void *)-1;
    }
...
static void record_backtrace()
{
    ...
    if (fd == -1) {
        write_log(LOG_ERR, "open[%s] failed with errno[%d]",
    ...
    if (n == ADDRESS_SIZE) {
        write_log(LOG_WARNING, "addresses of stack may have been truncated");
    }
...
static void clear_up_wd_maps(int inot_fd)
{
    ...
    for (std::map<int, std::string>::iterator it = wd_path_map.begin(),
        ...
        if (conveyor.empty()) {
            ...
            if (ret == -1) {
                write_log(LOG_ERR, "inotify_rm_watch[%s] for update failed"
    ...
    for (size_t i = 0, j = its.size(); i < j; i++) {
        ...
        write_log(LOG_INFO, "erasing wd[%d] path[%s] from wd_path_map",
                  it->first, it->second.c_str());
        wd_path_map.erase(it);
    }
    for (size_t i = 0, j = its2.size(); i < j; i++) {
        std::map<int, std::string>::iterator& it = its2[i];
        write_log(LOG_INFO, "erasing wd[%d] topic[%s] from wd_topic_map",
                  it->first, it->second.c_str());
...
static void clear_up_offset_table()
{
    ...
    for (size_t i = 0, j = its.size(); i < j; i++) {
        write_log(LOG_INFO, "erasing old key[%s] "
...
{
    write_log(LOG_INFO, "thread zero_update created");

    int ret = pthread_detach(pthread_self());
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_detach[zero_update] failed"
                  " with errno[%d]", ret);
        write_log(LOG_ERR, "thread zero_update exiting");
        return (void *)-1;
    }
    ...
    do {
        ...
        write_log(LOG_INFO, "updating some global data with "
                  "yesterday[%s] & today[%s]",
        ...
        for (std::multimap<std::string, std::string>::const_iterator it =
            ...
            if (wd == -1) {
                int errno_sv = errno;
                if (errno_sv != ENOENT) {
                    write_log(LOG_ERR, "inotify_add_watch[%s] for zero update"
                              " failed with errno[%d]",
                              it->second.c_str(), errno_sv);
                    close(inot_fd);
                    write_log(LOG_ERR, "thread zero_update exiting");
                ...
                write_log(LOG_WARNING, "inotify_add_watch[%s] for zero update"
                          " failed with errno[%d]",
                          it->second.c_str(), errno_sv);
            } else {
                wd_path_map[wd] = it->second;
...
{
    write_log(LOG_INFO, "thread routine_update created");

    int ret = pthread_detach(pthread_self());
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_detach[routine_update] failed"
                  " with errno[%d]", ret);
        write_log(LOG_ERR, "thread routine_update exiting");
        return (void *)-1;
    }
...
static void *remedy(void *arg)
{
    ...
    write_log(LOG_INFO, "thread remedy[%s] created", buf);

    int ret = pthread_detach(pthread_self());
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_detach[remedy] failed with errno[%d]", ret);
        write_log(LOG_ERR, "thread remedy[%s] exiting prematurely", buf);
        delete buf;
        return (void *)-1;
    }

    if ((p = strrchr(buf, ':')) == NULL) {
        write_log(LOG_ERR, "invalid offset record[%s] for remedy", buf);
        write_log(LOG_ERR, "thread remedy[%s] exiting prematurely", buf);
        delete buf;
        return (void *)-1;
    }
    *p = '\0';
    ...
    write_log(LOG_INFO, "thread remedy[%s][%d] is leaving", buf, num);
    delete buf;
...
static int check_and_remedy()
{
    ...
    if (stat(OFFSET_ARCHIVE_PATH, &st) == 0) {
        ...
        write_log(LOG_INFO, "remedy-ing unproduced messages");
        FILE *fp = fopen(OFFSET_ARCHIVE_PATH, "r");
        if (fp == NULL) {
            write_log(LOG_ERR, "fopen[%s] for remedy failed with errno[%d]",
                      OFFSET_ARCHIVE_PATH, errno);
            return -1;
        }
        char buf[128];
        ...
        while (fgets(buf, sizeof(buf), fp) != NULL) {
            ...
            if ((ret = pthread_create(&thr, NULL, remedy, (void *)s)) != 0) {
                write_log(LOG_ERR, "pthread_create[remedy] failed"
                          " with errno[%d]", ret);
                delete s;
                fclose(fp);
                exit(EXIT_FAILURE);
        ...
        if (errno != ENOENT) {
            write_log(LOG_ERR, "stat[%s] failed with errno[%d]",
                      OFFSET_ARCHIVE_PATH, errno);
            exit(EXIT_FAILURE);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        default:
            // usage
            fprintf(stderr, "Usage: ./log_aggregator -c -b -p\n");
            //exit(EXIT_FAILURE);
            continue;
        }
    } while (opt != -1);
...
for (std::map<std::string, std::string>::iterator it = raw_paths.begin();
    ...
    if (normal_paths.size() == 0) {
        write_log(LOG_ERR, "normalize_paths failed");
        exit(EXIT_FAILURE);
    }
#if 1
for (std::multimap<std::string, std::string>::iterator it =
    ...
    if ((producer = create_kafka_producer(producer_conf, brokers)) == NULL) {
        write_log(LOG_ERR, "create_kafka_producer to brokers[%s]"
...
for (std::map<int, std::string>::iterator it = wd_path_map.begin();
    ...
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_create[bar] failed with errno[%d]", ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
        exit(EXIT_FAILURE);
    ...
    pthread_t thr;
    if ((ret = pthread_create(&thr, NULL, foo, NULL)) != 0) {
        write_log(LOG_ERR, "pthread_create[foo] failed with errno[%d]", ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
        exit(EXIT_FAILURE);
    ...
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_create[archive_offsets] failed"
                  " with errno[%d]", ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
    ...
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_create[handle_conveyor_backlog] failed"
                  " with errno[%d]", ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
    ...
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_create[zero_update] failed with errno[%d]",
                  ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
    ...
    if (ret != 0) {
        write_log(LOG_ERR, "pthread_create[routine_update] failed"
                  " with errno[%d]", ret);
        destroy_topics();
        destroy_kafka_producer(producer);
        close(inot_fd);
    ...
    if ((ret = pthread_join(thr2, NULL)) != 0) {
        write_log(LOG_ERR, "pthread_join[bar] failed with errno[%d]", ret);
    }
    ...
    write_log(LOG_WARNING, "unexpected exiting");
